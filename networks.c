...
int tcpClientSetup(char *serverName, char *serverPort, int debugFlag) {
  ...
  if (connect(socket_num, (struct sockaddr *)&serverAddress,
              sizeof(serverAddress)) < 0) {
    perror("connect call");
    exit(-1);
  }
  ...
  if (debugFlag) {
    printf("Connected to %s IP: %s Port Number: %d\n", serverName,
           getIPAddressString6(ipAddress), atoi(serverPort));
  }

  return socket_num;
}

// This funciton creates a UDP socket on the server side and binds to that
// socket. It prints out the port number and returns the socket number.
int udpServerSetup(int serverPort) {
  ...
  // bind the name (address) to a port
  if (bind(socketNum, (struct sockaddr *)&serverAddress,
           sizeof(serverAddress)) < 0) {
  ...
  getsockname(socketNum, (struct sockaddr *)&serverAddress,
...
// This function opens a socket and fills in the serverAdress structure using
// the hostName and serverPort. It assumes the address structure is created
// before calling this. Returns the socket number and the filled in
...
// Just get a socket. Will bind to an ephemeral port
